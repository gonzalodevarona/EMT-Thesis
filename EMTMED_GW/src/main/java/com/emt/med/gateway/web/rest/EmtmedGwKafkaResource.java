...
@RestController
@RequestMapping("/api/emtmed-gw-kafka")
public class EmtmedGwKafkaResource {

    private final Logger log = LoggerFactory.getLogger(EmtmedGwKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public EmtmedGwKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
