...
import com.emt.med.gateway.config.EmbeddedKafka;
import com.emt.med.gateway.config.KafkaSseConsumer;
import com.emt.med.gateway.config.KafkaSseProducer;
...
class EmtmedGwKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/emtmed-gw-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/emtmed-gw-kafka/consume")
